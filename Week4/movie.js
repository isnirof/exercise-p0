...
function findMovie(arrMovie) {
  ...
  for (var i = 0; i < arrMovie.length; i++) {
    //split utk ambil tahun
    var tanggal = split(arrMovie[i][1], ' ')
    console.log(tanggal[2])
    if (result[tanggal[2]] == undefined) {
      result[tanggal[2]] = [{
        date: '',
        title: '',
        genre: '',
    ...
    else if (result[tanggal[2]]) {
      result[tanggal[2]].push({
        date: tanggal[0] + ' ' + tanggal[1],
        title: arrMovie[i][0],
        genre: arrMovie[i][2],
      })
    }
