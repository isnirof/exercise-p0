...
function kenaRazia(date, data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].type == 'Mobil') {
            var nomor = data[i].plat.split(' ')
            // var genap = (date % 2 == 0)
            // console.log(nomor)
            if ((Number(nomor[1]) % 2 == 0) !== (date % 2 == 0)) {
                for (var j = 0; j < lokasiRazia.length; j++) {
                    for (var k = 0; k < data[i].rute.length; k++) {
                        if (data[i].rute[k] == lokasiRazia[j]) {
                            tilang++
                        }
                    }
                }
