...
function train (pokemon, candy) {
  // Initial data
  var change=0
  var levelUp=candy+pokemon.level

  if(!pokemon.evolutionLine) {
      return `Congratulations, your ${pokemon.name} grew to level ${levelUp}`
  }
  for(var i=0;i<pokemon.evolutionLine.length;i++) {
      if(levelUp==pokemon.evolutionLine[i].level) {
          change=pokemon.evolutionLine[i].name
      }
  }
  return `Congratulations, your ${pokemon.name} grew to level ${levelUp}! And it evolved into ${change}`
}
