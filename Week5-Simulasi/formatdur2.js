...
function formatDuration(seconds) {
  ...
  while (sisa > 0) {
    for (div in format) {
      if (Math.floor(sisa / format[div]) > 0) {
        hasil.push([Math.floor(sisa / format[div]), div])
        // break
      }
      sisa = (sisa % format[div])
    }
  }
  var hasil2 = ''
  if (hasil.length == 1) {
    hasil2 += hasil[0][0] + ' ' + hasil[0][1]
  }
  for (var i = 0; i < hasil.length - 1; i++) {
    if (i == hasil.length - 2) {
      hasil2 += hasil[i][0] + ' ' + hasil[i][1] + ' dan ' + hasil[hasil.length - 1][0] + ' ' + hasil[hasil.length - 1][1]
    }
    else {
      hasil2 += hasil[i][0] + ' ' + hasil[i][1] + ', '
    }
