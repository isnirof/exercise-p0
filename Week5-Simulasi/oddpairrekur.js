...
function oddPairFirstLast(num) {
  ...
  if (str.length == 1) {
    var angka = Number(str[0] + 1)
  }
  else {
    var angka = Number(str[0] + str[str.length - 1])
  }
  
  var sisa = str.slice(1, str.length - 1)
  if (angka % 2 == 1) {
    return angka + oddPairFirstLast(sisa)
  }
  else {
    return oddPairFirstLast(sisa)
  }
}
