...
function profitCalculator(buyer) {
    ...
    var recipe = [
        { nama: 'Coldbrew', kopi: 2, sugar: 2, profit: 30000 },
        { nama: 'Coffeenyaa', kopi: 3, sugar: 1, profit: 5000 },
        { nama: 'Javachino', kopi: 1, sugar: 3, profit: 40000 }
    ]
    var stockKopi = 30
    var stockGula = 30
    var result = []
    for (var i = 0; i < buyer.length; i++) {
        ...
        for (var j = 0; j < recipe.length; j++) {
            // console.log(stockGula > 0 && stockKopi > 0, stockGula,stockKopi)
            if (stockGula > 0 && stockKopi > 0) {
                if (buyer[i].menu == recipe[j].nama) {
                    if (!obj.menu) {
                        obj.menu = buyer[i].menu
                        obj.time = [buyer[i].waktu]
                        obj.profit = recipe[j].profit*buyer[i].amount
                        obj.total = buyer[i].amount
                    }
                    else {
                        obj.time.push(buyer[i].waktu)
                        obj.profit += recipe[j].profit*buyer[i].amount
                        obj.total += buyer[i].amount
                    }
                    stockKopi -= recipe[j].kopi
                    stockGula -= recipe[j].sugar
                }
            }
        }
        result.push(obj)
