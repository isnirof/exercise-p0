...
function getReport(studentGrades) {
    ...
    for (var i = 0; i < studentGrades.length; i++) {
        ...
        for (var j = 0; j < hasil.length; j++) {
            if (hasil[j].classCode == studentGrades[i].classCode) {
                if (studentGrades[i].score > 70) {
                    hasil[j].lulus.push(studentGrades[i].name)
                }
                else {
                    hasil[j].gagal.push(studentGrades[i].name)
                }
                value = true
            }
        }
        if (!value) {
            ...
            if (studentGrades[i].score > 70) {
                obj.lulus.push(studentGrades[i].name)
            }
            else {
                obj.gagal.push(studentGrades[i].name)
            }
            hasil.push(obj)
        }
    }
