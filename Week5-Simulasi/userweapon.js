...
function userWeapon (user, weapon) {
  ...
  for(var i=0;i<heroes.length;i++) {
      if(user.job==heroes[i].job) {
          for(var j=0;j<heroes[i].weaponTypes.length;j++) {
              if(weapons[weapon].type==heroes[i].weaponTypes[j]) {
                  newAttack=user.attack-weapons[user.weapon].attack+weapons[weapon].attack
                  return `${user.name} menggunakan senjata ${weapon} attack menjadi ${newAttack}`
                }
          }
      }
  }
