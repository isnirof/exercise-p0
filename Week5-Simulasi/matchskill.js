...
function matchIn(applicant) {
    ...
    var comp = [
        { name: 'PT ABC', skill: ['XML', 'Kotlin', 'HTML', 'C++', 'Java', 'Javascript'], exp: 5 },
        { name: 'PT HIJ', skill: ['Python', 'Javascript', 'HTML'], exp: 3 },
        { name: 'PT JKL', skill: ['C++', 'Java'], exp: 1 },
        { name: 'PT XYZ', skill: ['Golang', 'Java', 'JQuery'], exp: 2 },
        { name: 'PT KLM', skill: ['Python', 'Javascript', 'HTML'], exp: 4 },
    ]
    var result = []
    for (var i = 0; i < comp.length; i++) {
        if (applicant.experience >= comp[i].exp) {
            var match = []
            for (var j = 0; j < applicant.skills.length; j++) {
                for (var k = 0; k < comp[i].skill.length; k++) {
                    if (applicant.skills[j] == comp[i].skill[k]) {
                        match.push(applicant.skills[j])
                    }
                }
            }
            // console.log(match)
